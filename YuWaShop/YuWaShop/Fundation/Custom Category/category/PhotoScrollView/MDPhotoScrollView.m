...
#define KScreenWidth   kScreen_Width
#define KScreenHeight  kScreen_Height
...
- (void)makeScrollViewWithSelectIndex:(NSInteger)selectIndex{
    ...
    for (int i = 0; i < self.picArr.count; i++) {
        [[SDWebImageManager sharedManager]downloadImageWithURL:[NSURL URLWithString:self.picArr[i]] options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            CGSize size = [JWTools getScaleImageSizeWithImageView:image withHeight:KScreenHeight withWidth:KScreenWidth];
